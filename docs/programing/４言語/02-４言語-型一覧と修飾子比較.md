...
```rust
let x: i32 = 10; let name: &str = "Alice";
```
...
|言語|記法|例|備考|
|---|---|---|---|
|C#|`T?`|`int? x = null;` / `string? s`|参照型の null 許容は nullable 参照型有効時（アノテーション）。|
|Rust|`Option<T>`|`let x: Option<i32> = None;`|`match`/`?` 演算子で扱う。|
|Python|`None`|`x: Optional[int] = None`|実体は `None`。型ヒントで `Optional[T]` を用いる。|
