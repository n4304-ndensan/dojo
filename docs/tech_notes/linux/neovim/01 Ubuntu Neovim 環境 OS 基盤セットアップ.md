...
Neovim 0.12 系の開発環境を Ubuntu 24.04 上に構築するための、OS レベルの準備手順を示します。ここで扱うのは OS パッケージ、言語別ツールチェーン（uv / rustup / dotnet）、Neovim 本体のインストールまでです。Neovim の設定ファイルには触れません。
...
sudo apt update
sudo apt install -y \
  curl git software-properties-common unzip build-essential \
  ripgrep fd-find fzf \
...
| `software-properties-common` | `add-apt-repository` を使って backports / PPA を追加する |
...
.NET SDK 10 は Ubuntu 24.04 の組み込み Ubuntu feed から導入できます。
...
> Ansible 版の `infra/ansible/ubuntu_dev` も、この前提に合わせて `dotnet-sdk-10.0` を既定値にしています。
...
Ubuntu 24.04 の `apt` 標準リポジトリにある Neovim は 0.9.5 です。0.12 系のネイティブ LSP 機能（`vim.lsp.config()` / `vim.lsp.enable()`）を使うため、公式リリースの `.deb` を直接インストールします。
...
出力に `NVIM v0.12` 以上が表示されることを確認してください。
